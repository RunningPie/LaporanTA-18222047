...
\begin{table}[!ht]
    ...
    \begin{tabular}{|p{2.5cm}|p{2cm}|p{2cm}|p{2cm}|p{2cm}|}
        ...
        \hline
        \hline
        Grup A (Intervensi) & 1313,33 & 1445,08 & 0,80 & 0,66 \\
        \hline
        Grup B (Kontrol) & 1300,00 & 1236,59 & -0,26 & -0,23 \\
        \hline
    \end{tabular}
\end{table}

\subsection{Analisis Signifikansi Peningkatan (NLG)}
Berdasarkan Tabel \ref{tbl:perbandingan_skor}, terlihat bahwa Grup A mengalami peningkatan hasil belajar yang sangat positif, dengan nilai NLG Teoretis sebesar $0,80$ dan NLG Empiris sebesar $0,66$. Merujuk pada kriteria interpretasi Hake, nilai ini tergolong ke dalam kategori peningkatan sedang hingga tinggi (\textit{medium to high gain}). Sebaliknya, Grup B justru mengalami penurunan performa dengan nilai NLG Teoretis $-0,26$ dan Empiris $-0,23$. Penurunan skor pada grup kontrol ini secara empiris mengindikasikan terjadinya kelelahan kognitif (\textit{cognitive fatigue}) atau demotivasi akibat terjebak berulang kali dalam soal-soal sulit (\textit{stagnasi}) tanpa adanya bantuan eksternal.
...
Meskipun algoritma utamanya terbukti terlalu sensitif, kombinasi dengan pemicu cadangan (\textit{fallback trigger} pada $N=8$ aktivitas terakhir) tetap memberikan jaring pengaman (\textit{safety net}) komprehensif. Secara konseptual, keberadaan \textit{fallback trigger} memastikan bahwa pengguna dengan profil skor yang sangat fluktuatif ke arah bawah akibat perilaku menebak asal, yang secara matematis luput dari deteksi \textit{variance} konvergen, tetap terjamin mendapatkan intervensi. Terlepas dari kelemahan sensitivitas \textit{trigger}-nya yang berlebih, intervensi proaktif dari mekanisme gabungan ini secara aktual terbukti berhasil memecah isolasi pembelajaran tanpa ada satu pun siswa Grup A yang luput dari pantauan sistem.

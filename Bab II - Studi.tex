...
untuk meningkatkan perolehan pembelajaran individu dan pengalaman pelajar dalam
...
adaptif untuk menilai keterampilan pelajar \autocite{Minn2022}. Menurut \textcite{Ihichr2024}, 
penilaian pelajar adalah salah satu momen utama dalam proses pendidikan.
...
yang menyesuaikan dan mempersonalisasi proses pembelajaran, terutama konten pembelajarannya, agar sesuai dengan situasi dan keadaan pelajar. Tujuan dari
...
kebutuhan dan perilaku pelajar. Pembelajaran adaptif memungkinkan
pelajar menjadi kolaborator dalam proses pendidikan, tidak hanya menjadi penerima informasi pasif. Hal ini dikarenakan setiap pelajar memiliki jalur pembelajaran optimal tersendiri yang terdiri dari banyak titik terhubung, dengan setiap titik mewakili komponen pengetahuan atau keterampilan \autocite{Ihichr2024}.

Pembelajaran adaptif mempertimbangkan banyak aspek, seperti: tingkat kemampuan pelajar saat ini, kebutuhan unik masing-masing pelajar, gaya belajar masing-masing pelajar, dan interaksi pelajar yang berbeda-beda, untuk mengindividualisasi semua komponen proses pembelajaran. Komponen proses pembelajaran yang dapat diindividualisasikan antara lain: konten, antarmuka, gaya belajar, dan asesmen. Oleh karena itu, pembelajaran adaptif memungkinkan seorang pelajar untuk berkembang ke pembelajaran yang lebih menantang berdasarkan kinerjanya, sambil tetap memberikan dukungan kepada pelajar lainnya untuk menguasai keterampilan tertentu. Pendekatan ini memastikan bahwa pelajar tidak dibatasi
oleh kecepatan kelas tetapi dapat mengikuti kecepatan belajar mereka sendiri.
Dengan demikian, pengajar dan pelajar sama-sama dapat membuat keputusan yang tepat pada saat yang tepat, dengan mengikuti kurva belajar dan lintasan belajar (\textit{learning path}). Singkatnya, pembelajaran adaptif berusaha untuk menyesuaikan pengalaman pembelajaran untuk membantu pelajar mencapai tujuan pembelajaran mereka dengan meniru interaksi personal antara seorang pengajar dan
seorang pelajar \autocite{Ihichr2024}.
...
pada asesmen daripada konten. Data asesmen yang dikumpulkan dari respons pelajar digunakan untuk mempersonalisasi proses pembelajaran, menciptakan jalur yang disesuaikan berdasarkan hasil. Jalur ini terus diperbarui dengan setiap asesmen.
Dengan demikian, asesmen pelajar dianggap sebagai faktor krusial untuk proses pembelajaran yang sukses \autocite{Ihichr2024}.
...
\begin{enumerate}
    ...
    pencapaian pelajar dan memverifikasi penguasaan kurikulum. Asesmen ini
    bertujuan menilai kemajuan kumulatif pelajar \autocite{Ihichr2024}. Informasi
    ...
    untuk memberikan informasi umpan balik kepada pengajar dan pelajar sehingga membantu
    ...
    pelajar, serta mengembangkan disiplin \autocite{Ihichr2024}.
    
    % \subsubsection{Pra-Asesmen}
    \item Praasesmen atau yang dikenal juga sebagai asesmen diagnostik adalah asesmen yang mengukur kompetensi awal setiap pelajar. Ini dilakukan untuk memberikan gambaran yang jelas kepada
    pengajar tentang tingkat keterampilan pelajar. Praasesmen biasanya dilakukan di awal pembelajaran dan dirancang
    untuk mengukur pengetahuan yang sudah diperoleh sebelumnya oleh pelajar sehingga bisa
    mengidentifikasi kebutuhan, kompetensi, dan pengetahuan sebelumnya
    dari pelajar tersebut. Identifikasi ini selanjutnya bisa membantu mengarahkan pelajar ke kurikulum yang paling sesuai
    \autocite{Ihichr2024}.
\end{enumerate}
...
\begin{enumerate}
    ...
    pelajar dari suatu topik, tetapi keterampilan intelektual menyangkut kemampuan yang
    berbeda, seperti penalaran, proses berpikir, dan pengambilan keputusan.

    % \subsubsection{Luaran Perilaku}
    \item \textbf{Luaran Perilaku:} luaran perilaku merujuk pada pengukuran keterlibatan pelajar dengan memantau
    ...
    \item \textbf{Luaran Afektif:} luaran ketiga ini berkaitan dengan persepsi pelajar, kepuasan mereka terhadap pembelajaran,
...
Integrasi kata kerja komputasi ini memungkinkan perancangan kompetensi-kompetensi yang lebih akurat. Kompetensi yang mengintegrasikan pengetahuan (\textit{knowledge}), keterampilan (\textit{skills}), dan disposisi (\textit{dispositions}) dalam konteks tertentu \autocite{ACM2023}. Dalam lingkungan pembelajaran adaptif, pemetaan ini krusial untuk melacak kemajuan pelajar. Seorang pelajar mungkin memulai pembelajaran pada level \textit{Applying} dengan menerjemahkan spesifikasi menjadi kode, namun seiring waktu diharapkan berkembang ke level \textit{Creating} dengan mengimplementasikan solusi untuk masalah abstrak. \textcite{ACM2023} juga menyoroti bahwa disposisi atau aspek sikap (seperti \textit{collaborative} dan \textit{meticulous}) dapat dikaitkan dengan kompetensi teknis yang dikembangkan tersebut. Dengan demikian, sistem asesmen tidak hanya melacak kebenaran sintaksis kode, tetapi juga kedalaman pemahaman konseptual dan kemampuan kolaboratif pelajar berdasarkan hierarki kognitif yang terstandardisasi.

\section{Pemodelan pelajar (\textit{Student Modeling})}
...
untuk membangun sebuah \textbf{\textit{student model}} (model pelajar). Model
pelajar adalah inti dari setiap sistem pendidikan adaptif, yang juga dikenal
...
pelajar saat ini. Selain pengetahuan, \textit{student model} juga dapat
merepresentasikan informasi lain tentang pelajar seperti tujuan pembelajaran
...
langsung, namun keberadaannya menentukan hasil atau kinerja pelajar pada
...
(\textit{unobservable}) oleh pelajar, dan hanya digunakan oleh sistem untuk
menggerakkan adaptasi. Namun, sebuah pendekatan yang
dikenal sebagai \textbf{\textit{Open Student Modeling}} (OSM) atau
Pemodelan pelajar Terbuka, berusaha membuat terobosan baru dengan secara sengaja
memperlihatkan aspek-aspek model tersebut kepada pelajar. Tujuan dari
pendekatan OSM adalah untuk meningkatkan refleksi diri, pembelajaran mandiri, transparansi personalisasi, dan motivasi bagi pelajar
...
Pemodelan pelajar Sosial Terbuka. Ide dari OSSM adalah untuk
melengkapi aspek kognitif OSM dengan aspek sosial.
Hal ini dicapai dengan memungkinkan pelajar mengeksplorasi model-model
rekan pelajar dan/atau model kelas yang diagregasi. Pendekatan ini memiliki kesamaan dengan teknik \textit{gamification} seperti \textit{leaderboards} (papan peringkat), yang bertujuan memanfaatkan perbandingan sosial untuk meningkatkan motivasi dan keterlibatan pelajar. Penelitian telah menunjukkan bahwa antarmuka OSSM, seperti yang diimplementasikan dalam \textit{MasteryGrids},
dapat meningkatkan pembelajaran, terutama untuk pelajar dengan pengetahuan awal
...
kompetensi pelajar dan tingkat kesulitan konten pembelajaran \autocite{Vesin2022}.
...
pelajar, mengembangkan pengukuran kognitif atau non-kognitif pelajar, memilih 
...
pelajar statis selama ujian. Keadaan pengetahuan pelajar $\theta$ dinilai berdasarkan
...
karena mengasumsikan pelajar tidak belajar selama proses pengujian sehingga lebih sesuai dianggal sebagai pendekatan \textit{Knowledge Assessment}.
Salah satu kelemahan IRT menurut \textcite{Vesin2022} adalah dalam hal kalibrasi pada sampel besar dan sulitnya melakukan komputasi untuk pembaruan yang sering karena estimasi baru terhadap keterampilan harus dihitung untuk setiap pelajar setelah satu respons untuk satu tugas.
...
pelajar dapat diperkirakan dari kinerjanya (\textit{performance}) pada tugas-tugas
...
semua respons yang diberikan oleh pelajar i pada \textit{item} j pada waktu t. Dalam penelitian oleh \textcite{Minn2022},
...
% pengetahuan, keadaan tersembunyi adalah penguasaan pelajar terhadap suatu
...
% Asumsi kedua adalah bahwa observasi (misalnya, jawaban pelajar) pada waktu $t$
% hanya bergantung pada keadaan pada waktu $t$. Dalam model
% ini, keadaan pengetahuan pelajar dimodelkan sebagai variabel laten yang bertransisi
% antar keadaan (misalnya, dari "belum dipelajari" ke "dipelajari"), dan
% probabilitas transisi ini diperbarui berdasarkan urutan kinerja pelajar
...
% asesmen adaptif, FA utamanya berfungsi untuk membangun model pelajar,
...
% pelajar dengan menghitung probabilitas penguasaan keterampilan berdasarkan 
...
memodelkan keadaan pengetahuan pelajar yang berubah dan merupakan model pertama 
...
pelajar disimpulkan pada setiap waktu berdasarkan hasil sebelumnya. Pendekatan ini relevan bagi pengajar yang memanfaatkan latihan dan metode \textit{scaffolding} sebagai upaya utama dalam pembelajaran.
...
untuk memodelkan proses pembelajaran pelajar dan melacak pengetahuan mereka. DKT digunakan untuk
...
model yang dibangun juga dapat digunakan untuk merancang asesmen berikutnya yang lebih sesuai untuk pelajar \autocite{Ihichr2024}.
...
untuk merepresentasikan keadaan pengetahuan laten dengan struktur temporal yang dinamis. Representasi ini yang memungkinkan model untuk mempelajari keadaan pengetahuan pelajar dari data. Namun,
...
celah yang ditimbulkan oleh teknik pemodelan pelajar yang lainnya. \textit{Elo rating} sepenuhnya bergantung
pada evaluasi kuantitatif kinerja pelajar dan dapat memodelkan keterampilan yang
...
dan menetapkan tingkat kesulitan untuk konten pembelajaran berdasarkan jawaban pelajar yang mengerjakan soal tersebut. Algoritma \textit{Elo rating} juga dapat memberikan estimasi
...
bidang pendidikan mengasumsikan bahwa pelajar adalah pemain dan \textit{item} konten
(misalnya latihan penulisan kode program) adalah lawan \autocite{Vesin2022}.

Peringkat \textit{Elo} pelajar
dan materi pemebalajaran diwakili oleh angka yang meningkat atau menurun tergantung pada interaksi pelajar dengan materi pembelajaran. Perbedaan peringkat antara pelajar dan \textit{item} pembelajaran berfungsi sebagai prediktor hasil interaksi. Algoritma \textit{Elo rating} merupakan algoritma yang hemat sumber daya komputasi, sangat sederhana untuk diimplementasikan, dan cocok untuk praktik adaptif dalam pendidikan. Implementasi
...
Secara spesifik, metode yang diusulkan menerapkan dua strategi, (1) memperbarui peringkat pelajar
berdasarkan performa pelajar pada interaksinya dengan materi pembelajaran dibandingkan nilai interaksinya dan (2)
tingkat kesulitan konten pembelajaran diperbarui berdasarkan peringkat atau skor pelajar yang berhasil ataupun gagal dalam berhadapan dengan konten tersebut. Secara matematis, cara ini mengestimasi probabilitas seorang pelajar \textit{i} mampu menyelesaikan latihan \textit{j} berdasarkan peringkat pelajar saat ini dan tingkat kesulitan latihan \textit{j}.
Peringkat pelajar dan kesulitan latihan kemudian diperbarui menurut rumus berikut,

\begin{align}
R_i &= R_{i-1} + K (W - W_e)\label{eq:rumus_elo_student}
\end{align}
\eqcaption{Perubahan Peringkat pelajar (Ri)}
\begin{align}
D_j &= D_{j-1} + K (W_e - W)\label{eq:rumus_elo_latihan}
\end{align}
\eqcaption{Perubahan Tingkat Kesulitan Soal (Dj)}

dengan $R_i$ adalah peringkat pelajar setelah menyelesaikan latihan ke-i,
$R_{i-1}$ adalah peringkat pelajar sebelum menyelesaikan latihan ke-i,
$D_j$ adalah tingkat kesulitan latihan ke-j setelah pelajar menyelesaikannya,
$D_{j-1}$ adalah tingkat kesulitan latihan ke-j sebelum pelajar menyelesaikannya,
$K$ adalah faktor penyesuaian yang menentukan seberapa besar perubahan peringkat
atau tingkat kesulitan, nilai ini dikurangi secara bertahap seiring pelajar menyelesaikan lebih banyak latihan,
...
seorang pelajar mampu menyelesaikan latihan tertentu, yang dihitung berdasarkan rumus \textit{Elo} standard \autocite{Elo1978}:
\begin{align}
...
berpusat pada pelajar tunggal. Sebuah contoh representatif dari alur kerja ini
...
\begin{enumerate}
    \item \textbf{Sesi Pembelajaran (\textit{Learning Sessions}):} pelajar berinteraksi
    dengan sumber daya pembelajaran yang disediakan, seperti membaca materi
    atau menyelesaikan latihan pengodean.
    \item \textbf{Asesmen Adaptif (\textit{Adaptive Assessment}):} Saat pelajar
    menyelesaikan aktivitas, sistem memperbarui peringkat mereka berdasarkan
    kinerja individu. Peringkat pelajar
    (\textit{student ratings}) dan tingkat kesulitan konten
    (\textit{difficulty level of content}) dihitung menggunakan metode \textit{Elo rating} yang memanfaatkan rumus \ref{eq:rumus_elo_student} dan rumus \ref{eq:rumus_elo_latihan}.
...
% keberhasilan pelajar murni berdasarkan parameter individual, seperti
...
Implementasi algoritma personalisasi dalam sistem pembelajaran adaptif bertujuan untuk mengoptimalkan efisiensi akuisisi pengetahuan dengan menyesuaikan materi terhadap profil unik setiap pelajar. Namun, optimasi yang terlalu agresif terhadap parameter individu tanpa mempertimbangkan aspek eksplorasi dan keberagaman informasi dapat memicu fenomena \textit{overpersonalization} \autocite{Halkiopoulos2024}. Risiko ini muncul ketika sistem menciptakan lingkungan yang terlalu tertutup, sehingga membatasi paparan pelajar terhadap konsep-konsep baru yang krusial bagi pengembangan kognitif yang holistik. Subbab berikut akan mengulas manifestasi dari kondisi tersebut serta metrik yang digunakan untuk mengukurnya.

\subsection{Manifestasi \textit{Overpersonalization} dalam Pembelajaran}

Manifestasi dari \textit{overpersonalization} dalam lingkungan pendidikan dapat diamati melalui perubahan perilaku kognitif pelajar saat berinteraksi dengan sistem yang terlalu terpersonalisasi. Menurut penelitian \textcite{Bahg2025}, pelajar dalam ekosistem kognitif yang membatasi keberagaman informasi cenderung melakukan sampling informasi secara lebih selektif. Hal ini menyebabkan pembentukan representasi kategori (materi pembelajaran) yang tidak tepat karena pelajar hanya terpapar pada subset data yang sempit dan homogen. Secara fundamental, fenomena ini mengindikasikan bahwa dampak negatif \textit{overpersonalization} tidak hanya terbatas pada isolasi informasi, tetapi juga menyebabkan distorsi pemahaman, bias dalam pengambilan sampel informasi, serta generalisasi pengetahuan yang tidak tepat \autocite{Bahg2025}. Dampak ini diperparah oleh pembentukan \textit{filter bubble} yang, sebagaimana ditunjukkan oleh \textcite{Jiang2025}, menyebabkan penurunan diversifikasi konsumsi informasi secara signifikan, yang pada akhirnya membatasi cakrawala intelektual pelajar.

\subsection{Pengukuran \textit{Overpersonalization} dalam Pembelajaran}

Pengukuran terhadap tingkat \textit{overpersonalization} memerlukan metrik yang mampu menangkap aspek keberagaman dalam rekomendasi konten edukasi. Dalam domain sistem rekomendasi secara umum, \textcite{Ren2025} menyatakan bahwa pengukuran tingkat keberagaman dapat dilakukan menggunakan metrik variansi atau entropi, seperti \textit{Shannon Entropy}, untuk menguantifikasi seberapa luas sebaran entitas dan relasi yang dikonsumsi oleh pengguna dibandingkan dengan total ruang pengetahuan yang tersedia. Dalam konteks spesifik pembelajaran, indikasi \textit{overpersonalization} juga dapat dideteksi melalui kemunculan fenomena \textit{learning plateau} \autocite{Halkiopoulos2024}. Fenomena ini ditandai dengan stagnasi kemajuan belajar (kognitif) pelajar yang disebabkan oleh kurangnya variasi terhadap tantangan atau tingkat kesulitan yang dihadapi. Ketika sistem terus-menerus menyajikan materi yang terlalu selaras dengan zona nyaman pelajar tanpa memberikan paparan terhadap konsep-konsep baru yang beragam, pelajar kehilangan stimulasi kognitif yang diperlukan untuk keluar dari stagnasi tersebut.
...
Pendekatan ini sangat relevan untuk konteks pembelajaran adaptif karena pengguna sensitif dapat dipetakan sebagai pelajar yang terisolasi secara akademik, dan \textit{favorable items} sebagai interaksi dengan rekan yang memiliki kompetensi berbeda (heterogen).
...
\input{algorithms/reranking-debiasio.tex}
...
berpusat pada pelajar \autocite{Ryoo2021} sejalan dengan penekanan Strategi Nasional
...
\begin{enumerate}
    ...
    tim (\textit{team-based projects}) memungkinkan pelajar untuk berinteraksi
...
\begin{enumerate}
    ...
    pelajar saling menilai pekerjaan satu sama lain, artinya dalam
    ...
    balik yang dihasilkan oleh pelajar.
    ...
    \textcite{Kerman2024} menemukan bahwa pelajar yang sukses
    ...
    diterima oleh pelajar yang kurang berhasil. Hal ini
    menunjukkan bahwa untuk meningkatkan kompetensi, sistem
    harus mendorong pelajar untuk memberikan komentar yang
    ...
    berpusat pada pelajar, dengan tujuan menghasilkan sebuah karya proyek yang memecahkan masalah dunia nyata \autocite{Zhang2023}. Berbeda
    dengan pembelajaran tradisional, PBL menempatkan pelajar dalam peran aktif untuk
    ...
    menemukan bahwa PBL secara signifikan meningkatkan hasil belajar pelajar
    ...
    yang diberikan oleh pelajar senior untuk membantu pelajar lain \autocite{Le2024}.
    Aktivitas ini dirancang untuk membantu pelajar baru (\textit{mentee}) dalam
    ...
    kegiatan ekstrakurikuler untuk meningkatkan kemampuan komunikasi lisan pelajar.
    Hasil studi menunjukkan adanya perkembangan signifikan dalam kemampuan berbicara
    pelajar. Keberhasilan ini adalah efek dari faktor-faktor kunci dalam desain aktivitas tersebut, yaitu penyediaan lingkungan
    ...
    metode ini, pelajar diminta untuk mengambil peran spesifik untuk mensimulasikan
    ...
    signifikan (\textit{ES}=0.818) pada pelajar dibandingkan dengan kelompok kontrol
    ...
    peningkatan keterampilan pelajar \autocite{Fu2025}.
\end{enumerate}
...
\autocite{Ihichr2024}. Studi oleh \textcite{Ihichr2024} mencatat bahwa pelajar mendapat lebih banyak manfaat dari bertindak sebagai penilai daripada hanya dinilai.
...
menyebabkan pelajar yang mendapat manfaat dari pembelajaran
bersama (\textit{shared learning}) akan terasingkan dalam sistem.
\textit{Overpersonalization} ini dapat menjadi penghalang ketika pelajar seharusnya mampu memperoleh manfaat dari keberagaman perspektif dalam komunitas pembelajarannya. Oleh karena itu,
...
dominan pada pelajar individu. Sistem \textit{e-learning} yang ada, sering kali
dirancang untuk mengadaptasi konten dan urutan penyampaiannya sesuai dengan
kemampuan pelajar secara individual \autocite{Halkiopoulos2024}. Fokus pada
...
eksplorasi sosial ini dapat meningkatkan pembelajaran, terutama bagi pelajar dengan pengetahuan awal rendah, pendekatan ini pada hakekatnya bersifat visualisasi. \textcite{Brusilovsky2016} menempatkan aspek sosial sebagai \textit{output} untuk memotivasi pelajar.
...
Penggunaan kuis daring dengan fitur percobaan berulang (\textit{multiple attempts}) merupakan bagian dari asesmen formatif yang bertujuan untuk meningkatkan proses berpikir dan pemahaman konsep pelajar terhadap materi pembelajaran. Menurut \textcite{Mendoza2022}, pemberian kesempatan bagi pelajar untuk mengerjakan soal kuis lebih dari satu kali memungkinkan mereka untuk mengenali celah dalam pemahaman kurikulum serta memantau progres belajar secara mandiri selama proses berlangsung. Hasil penelitian menunjukkan bahwa rata-rata nilai pada kuis daring dengan percobaan berulang terbukti menjadi prediktor yang signifikan terhadap pencapaian nilai pelajar pada ujian akhir. Selain itu, pelajar cenderung menunjukkan peningkatan skor yang signifikan dan waktu pengerjaan yang lebih singkat pada percobaan terakhir dibandingkan dengan percobaan pertama mereka setelah mendapatkan intervensi. Implementasi mekanisme ini juga terbukti memotivasi pelajar untuk menyelesaikan tugas persiapan, meningkatkan partisipasi dalam diskusi kelas, dan secara umum meningkatkan kinerja akademik pada ujian sumatif.

Dalam konteks jumlah upaya yang diberikan, penggunaan batasan tiga kali percobaan (\textit{three attempts}) teridentifikasi memiliki dampak positif yang signifikan terhadap peningkatan nilai kuis daring pelajar secara konsisten. \textcite{Mendoza2022} menjelaskan bahwa pada percobaan kedua dan ketiga, pelajar diwajibkan untuk menjawab kembali seluruh butir soal guna memperbaiki kesalahan dari percobaan sebelumnya sekaligus memastikan penguasaan konsep pada jawaban yang sudah benar. Persyaratan untuk menunjukkan solusi lengkap pada setiap percobaan bertujuan untuk mencegah pelajar melakukan penebakan jawaban secara sembarangan (\textit{guessing}) dan mendorong mereka untuk benar-benar belajar dari umpan balik yang diberikan sistem. Analisis kuantitatif melalui uji ANOVA menunjukkan bahwa terjadi peningkatan rerata nilai yang konsisten di seluruh instrumen kuis yang diujikan dalam kerangka tiga kali percobaan tersebut. Oleh karena itu, batasan tiga kali upaya ini dipandang mampu memberikan ruang yang cukup bagi pelajar untuk melakukan evaluasi diri dan melakukan penyuntingan terhadap pekerjaan mereka sendiri (\textit{self-edit}).

\subsection{Sistem Umpan Balik Otomatis (\textit{Automated Feedback System})}
\label{ssec:sistem_umpan_balik_otomatis}

Integrasi sistem umpan balik otomatis dalam kuis daring merupakan elemen krusial untuk mengatasi keterbatasan pengajar dalam memberikan koreksi secara instan dan berkelanjutan pada kelas dengan jumlah pelajar yang besar. \textcite{Mendoza2022} menyatakan bahwa umpan balik yang diberikan secara tepat waktu, spesifik, dan berkelanjutan sangat penting untuk memperbaiki kesalahan serta kekeliruan pelajar selama proses asesmen berlangsung agar tidak terulang kembali. Penggunaan teknologi digital dalam penyampaian umpan balik berpotensi meningkatkan kualitas respons serta menghemat waktu pemeriksaan bagi pengajar dibandingkan dengan metode tertulis tradisional. Umpan balik yang efektif harus memiliki karakteristik yang mudah dipahami, selektif, spesifik, tepat waktu, kontekstual, dan berorientasi ke depan agar dapat memicu tindakan perbaikan lebih lanjut dari sisi pelajar. Selain itu, umpan balik elektronik dapat mengurangi hambatan logistik sambil tetap mempertahankan fitur-fitur terbaik dari umpan balik tradisional yang bersifat jelas dan komprehensif bagi penerimanya.

Dampak dari sistem umpan balik otomatis ini tercermin pada persepsi positif pelajar yang menganggapnya sebagai panduan yang berguna dalam memahami materi pembelajaran meskipun tanpa kehadiran fisik pengajar secara langsung. Berdasarkan temuan \textcite{Mendoza2022}, mayoritas pelajar merasa terbantu dalam mengidentifikasi aspek materi yang belum dikuasai serta keterampilan spesifik yang perlu mereka tingkatkan melalui pesan koreksi yang muncul setelah setiap jawaban diberikan. Umpan balik yang dirancang dengan mempertimbangkan alasan masuk akal dari pilihan jawaban salah (\textit{plausible distractors}) memungkinkan pelajar untuk melakukan komputasi ulang dan mengingat kembali konsep yang telah diajarkan sebelumnya. Penting untuk dicatat bahwa umpan balik dalam sistem ini idealnya bersifat menyarankan (\textit{suggestive}) dan bukan memerintah (\textit{prescriptive}), sehingga pelajar tetap memiliki otonomi dan kebebasan dalam menentukan cara pemecahan masalah sesuai preferensi mereka sendiri. Dengan demikian, ketersediaan umpan balik yang instan bersamaan dengan fitur percobaan berulang menciptakan ruang interaksi yang bermakna antara pelajar dan sistem untuk meminimalkan stagnasi pembelajaran.
...
disebabkan oleh perbedaan tingkat pengetahuan awal pelajar. Untuk mengatasi hal
...
antara populasi pelajar yang memiliki latar belakang kemampuan yang heterogen
\autocite{Nissen2018}.
...
mengakibatkan pelajar dengan
skor awal tinggi memiliki ruang peningkatan yang lebih sempit dibandingkan pelajar
dengan skor rendah.

...
\begin{spacing}{1.0}
Perkembangan sistem asesmen adaptif bertujuan untuk mengoptimalkan jalur belajar individu melalui personalisasi konten. Namun, ketergantungan pada algoritma personalisasi yang agresif berisiko memicu fenomena \textit{overpersonalization}, yang menciptakan \textit{filter bubbles} akademik dan mengisolasi pembelajar dari keragaman perspektif. Kondisi ini dapat menyebabkan pengambilan sampel informasi yang selektif serta stagnasi kognitif (\textit{learning plateau}). Penelitian ini bertujuan untuk merancang dan mengimplementasikan purwarupa sistem asesmen adaptif kolaboratif bernama Equilibria yang mengintegrasikan fitur sosial sebagai upaya mitigasi terhadap efek \textit{overpersonalization}. 

Sistem diimplementasikan menggunakan arsitektur \textit{client-server} dengan algoritma \textit{Modified Elo Rating} untuk pelacakan kemampuan kognitif. Mekanisme mitigasi diwujudkan melalui deteksi stagnasi berbasis variansi perubahan skor ($\Delta\theta$) dan algoritma \textit{Constraint-Based Re-ranking} untuk memasangkan siswa dengan sejawat yang memiliki tingkat kompetensi heterogen ($d \geq 0,5$). Evaluasi dilakukan melalui \textit{Controlled Lab Study} yang melibatkan 10 partisipan dalam kelompok intervensi dan kontrol pada domain pembelajaran SQL.

Hasil pengujian menunjukkan bahwa kelompok intervensi mencapai peningkatan hasil belajar yang signifikan dengan nilai \textit{Normalized Learning Gain} (NLG) antara 0,57 hingga 0,69 (kategori sedang ke tinggi), sementara kelompok kontrol mengalami penurunan performa (NLG -0,21). Analisis trajektori kemahiran membuktikan bahwa intervensi kolaboratif berhasil mengembalikan momentum pertumbuhan belajar dengan rata-rata perbedaan gradien positif sebesar +0,67 setelah titik stagnasi terdeteksi. Algoritma \textit{re-ranking} juga terbukti efektif menciptakan pasangan heterogen dengan rata-rata \textit{Cohen's d} sebesar 1,186. Penelitian ini menyimpulkan bahwa integrasi intervensi sosial yang proaktif dan terautomasi merupakan strategi efektif untuk memecah isolasi akademik dalam lingkungan pembelajaran adaptif.

\vspace{0.4cm}
\noindent \textbf{Kata kunci:} Asesmen Adaptif, Pembelajaran Kolaboratif, \textit{Elo rating}, \textit{overpersonalization}, Filter Bubble.
\end{spacing}

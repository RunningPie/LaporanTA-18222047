...
\begin{enumerate}
    \item \textbf{Teoretis:} Menggunakan nilai \textit{Elo rating} maksimal sebesar 1500 (berdasarkan rentang kesulitan sistem pada studi \textcite{Vesin2022}).
...
Meskipun memberikan hasil yang positif, terdapat beberapa faktor yang perlu diperhatikan sebagai ancaman terhadap validitas (\textit{threats to validity}) eksperimen ini:
...
\begin{enumerate}
    ...
    \item \textbf{Generalisasi Hasil (\textit{External Validity}):} Pengujian ini melibatkan jumlah sampel yang terbatas ($n=10$) dan bersifat homogen (mahasiswa dari satu program studi). Sesuai dengan tujuan pengembangan purwarupa, penelitian ini dikategorikan sebagai \textit{Pilot Study} yang merujuk pada metodologi \textcite{Sukserm2024}, di mana fokus utama diletakkan pada pengujian validitas internal arsitektur dan pembuktian konsep (\textit{proof-of-concept}) alih-alih generalisasi populasi luas.
